-- TPC-H 22 queries (standard text). Unsupported shapes fall back to CPU via mlx_execution.
-- Query numbers are embedded as comments for the runner.
...
WHERE p_partkey = ps_partkey AND s_suppkey = ps_suppkey AND p_size = 15
  AND ps_suppkey = s_suppkey AND p_type LIKE '%BRASS' AND s_nationkey = n_nationkey
...
FROM (
    ...
    WHERE s_suppkey = l_suppkey AND ps_suppkey = l_partkey AND ps_partkey = l_partkey
...
WHERE p_partkey = l_partkey AND p_brand = 'Brand#23' AND p_container = 'MED BOX'
  AND l_quantity < (
    ...
    WHERE l_partkey = p_partkey AND l_shipdate >= DATE '1995-01-01' AND l_shipdate < DATE '1996-01-01');
